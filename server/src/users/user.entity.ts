...
export class Users {
  ...
  getJwtPayload(): Users {
    this.password = 'la-la_you_find_password'; // Dummy assignment to avoid undefined password issue
    return {...this};
  }

  async generateToken(jwtService: JwtService): Promise<string> {
    const password = 'la-la_you_find_password'; // Dummy assignment to avoid undefined password issue
    return await jwtService.signAsync({ ...this, password });
  }
